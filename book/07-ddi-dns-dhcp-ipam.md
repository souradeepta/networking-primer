...
1. **Why can a DHCP client not simply send a unicast request at startup?** **Answer:** It may have no address, server address, or route. The initial broadcast lets a relay and server discover the client’s subnet.
...
2. **What does a relay add?** **Answer:** It carries the request across a routed boundary and supplies subnet context so the server can select the appropriate scope. It does not become the lease authority.
...
3. **Does a DHCPACK prove DNS works?** **Answer:** No. The ACK proves the server accepted a lease and returned options; resolver reachability, zone data, and delegation still need testing.
...
4. **Why is IPAM not automatically authoritative for every address?** **Answer:** IPAM usually records intended allocation and ownership, while DHCP records dynamic lease state. Their authority depends on the agreed data model.
...
5. **What is drift?** **Answer:** Drift is disagreement between systems or between recorded intent and observed state, such as an IPAM-free address that DHCP leases or a DNS name pointing to a retired host.
...
6. **Why can deleting a stale DNS record be dangerous?** **Answer:** It may be owned by another service, be temporarily hidden by caching, or be the only name for a still-live endpoint. Establish ownership and evidence first.
...
7. **What does a TTL control?** **Answer:** It bounds how long a caching resolver may reuse an answer; it does not control DHCP leases or guarantee instant worldwide change.
...
8. **How should conflicts be remediated?** **Answer:** Preserve evidence, quarantine the conflicting object, identify field owners, apply the smallest reversible correction, and verify from multiple vantage points.
