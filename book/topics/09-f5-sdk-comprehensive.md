...
1. **What is ManagementRoot?** **Answer:** It is the SDK entry point carrying management
...
2. **Why does `verify=True` matter?** **Answer:** It asks the client to validate the
...
3. **Why is partition awareness essential?** **Answer:** Similar names can exist in
...
4. **Is a 200 response proof of completion?** **Answer:** No. Read back state and relevant
...
5. **When is retrying a write safe?** **Answer:** Only when server semantics and the
...
6. **What should a 404 trigger?** **Answer:** Check version, partition, folder, and
...
7. **Why mock the SDK?** **Answer:** To prove safety and error handling without changing a
...
8. **What belongs in an audit record?** **Answer:** Target, scope, actor reference,
