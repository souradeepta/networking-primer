...
1. **Is LTM the same as BIG-IP?** **Answer:** No. LTM is a BIG-IP module/service; BIG-IP
...
2. **What is a VIP?** **Answer:** It is the client-facing address and service binding of a
...
3. **What is a node versus a member?** **Answer:** A node is a host object; a member is a
...
4. **Does an up monitor prove the application works?** **Answer:** No. It proves only that
...
5. **Why can the app see a BIG-IP address?** **Answer:** SNAT deliberately replaces the
...
6. **Does X-Forwarded-For undo SNAT?** **Answer:** No. It conveys an HTTP identity header;
...
7. **Where does TLS terminate?** **Answer:** At every profile boundary configured to
...
8. **Can persistence override load balancing?** **Answer:** Yes. A valid persistence
...
9. **Why might a 503 occur with healthy servers?** **Answer:** The pool may be unavailable
...
10. **What tuple should a capture show?** **Answer:** The client leg targets the VIP; the
...
11. **What should be checked first for a TLS alert?** **Answer:** Identify the failing leg,
...
12. **When is an iRule appropriate?** **Answer:** When a reviewed, tested event-driven rule
