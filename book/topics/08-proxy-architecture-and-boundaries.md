...
1. **What is the key difference between L4 and L7?** **Answer:** L4 uses transport
   metadata and connection state; L7 understands application messages. The distinction is about which fields and state the proxy can inspect or change.
...
2. **When is passthrough useful?** **Answer:** When end-to-end payload privacy or backend
...
3. **Why re-encrypt after termination?** **Answer:** To create a protected, authenticated
...
4. **Can every POST be retried?** **Answer:** No. A retry can duplicate side effects unless
...
5. **Who should set X-Forwarded-For?** **Answer:** A trusted boundary should sanitize and
...
6. **Why use a total deadline?** **Answer:** Independent hop timers can exceed the client
...
7. **What does a client 504 prove?** **Answer:** Only that the client-facing boundary
