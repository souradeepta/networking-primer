...
1. **Is a VIP the same as a virtual server?** **Answer:** Usually a VIP means the listener
...
2. **Why can a member answer while the VIP fails?** **Answer:** The direct test bypasses
   listener policy, profiles, SNAT, monitors, and return-path behavior.

Interview reasoning: For “Why can a member answer while the VIP fails,” distinguish the address from the LTM listener contract: the virtual server owns profiles, policies, pool selection, SNAT, and persistence. Trace a client tuple to the VIP and a second tuple to the member, then compare direct-member and VIP tests. The caveat is that a reachable VIP can still have no eligible pool member or an incorrect route domain.

3. **What does a pool do?** **Answer:** It supplies eligible destination members and a
...
4. **When is SNAT risky?** **Answer:** It can hide client identity and consume finite
...
5. **What does a monitor prove?** **Answer:** Only that its specific probe received its
...
6. **Why check profiles?** **Answer:** TCP, HTTP, TLS, and persistence profiles can alter
...
7. **What is the safest first action?** **Answer:** Collect read-only state and a narrowly
