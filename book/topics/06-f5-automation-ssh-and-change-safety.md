...
1. **Is SSH inherently safer than REST?** **Answer:** No. Safety depends on identity,
...
2. **What is idempotency?** **Answer:** Repeating the same desired-state request converges
...
3. **Why capture pre-state?** **Answer:** It supports review, precise diffs, and rollback. A reviewable diff exposes unintended changes before the write, and rollback restores the prior state.
...
4. **Why verify TLS in an API client?** **Answer:** Without server identity validation, a
...
5. **What should happen after an uncertain timeout?** **Answer:** Read back state and audit
...
6. **Why serialize changes?** **Answer:** Concurrent edits can invalidate assumptions and
   produce split or partial configuration. A lock or serialized queue prevents stale reads from producing conflicting writes.
...
7. **What is a useful post-change check?** **Answer:** Object read-back plus health signal
