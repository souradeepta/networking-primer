...
1. **Why are metrics not enough?** **Answer:** Metrics show aggregate behavior but often
...
2. **Should a DNS failure count against an HTTP SLO?** **Answer:** It depends on the
...
3. **What does an F5 health monitor prove?** **Answer:** It proves that the configured
...
4. **Why use percentiles instead of averages?** **Answer:** Averages hide tail latency,
...
5. **What is an error budget?** **Answer:** It is the tolerated unreliability implied by
...
6. **How can a trace cross an F5 proxy?** **Answer:** Preserve or generate a correlation
...
7. **Why normalize time?** **Answer:** Without synchronized clocks, event order and
...
8. **When is a packet capture appropriate?** **Answer:** When counters and transaction
