...
1. **What does `/24` mean?** **Answer:** It means 24 leading bits are the network prefix and eight bits remain for addresses. The dotted mask is usually `255.255.255.0`.
...
2. **Why is `/26` not simply four times smaller than `/24` in every sense?** **Answer:** It has one quarter as many addresses, 64 instead of 256, because two additional prefix bits leave four times fewer host combinations.
...
3. **What is longest-prefix matching?** **Answer:** It chooses the matching route with the greatest prefix length, which represents the narrowest destination range.
...
4. **What is a default route?** **Answer:** IPv4 `0.0.0.0/0` and IPv6 `::/0` match destinations not covered by a more specific route. They are useful exits, not proof that the destination is reachable.

Interview reasoning: For “What is a default route,” name the source and destination prefixes, longest-match decision, next hop, VRF or policy table, and return route. Verify both directions with route lookups and a narrow flow trace. A present route is not proof of ARP/ND, ACL, MTU, or listener success; NAT and proxies can also make endpoint evidence differ from the original client.

5. **Does NAT route packets?** **Answer:** A NAT device must route or bridge traffic as appropriate, but translation itself rewrites addresses or ports; it is not a route-distribution protocol.

Interview reasoning: For “Does NAT route packets,” name the source and destination prefixes, longest-match decision, next hop, VRF or policy table, and return route. Verify both directions with route lookups and a narrow flow trace. A present route is not proof of ARP/ND, ACL, MTU, or listener success; NAT and proxies can also make endpoint evidence differ from the original client.

6. **Why does IPv6 need Neighbor Discovery?** **Answer:** Nodes use ICMPv6 ND to discover link-layer neighbors, routers, and prefixes because IPv6 has no ARP broadcast mechanism.
...
7. **Why can a more specific bad route be dangerous?** **Answer:** It wins over a correct aggregate or default route, so traffic can be consistently diverted without an obvious missing-route error.

Interview reasoning: For “Why can a more specific bad route be dangerous,” name the source and destination prefixes, longest-match decision, next hop, VRF or policy table, and return route. Verify both directions with route lookups and a narrow flow trace. A present route is not proof of ARP/ND, ACL, MTU, or listener success; NAT and proxies can also make endpoint evidence differ from the original client.

8. **What evidence distinguishes a host problem from a router problem?** **Answer:** The host route table, interface counters, and local ARP/ND state show the host decision; router tables and captures on both interfaces show forwarding beyond it. [Inference: Correlating both sides is stronger than relying on a single ping.]
