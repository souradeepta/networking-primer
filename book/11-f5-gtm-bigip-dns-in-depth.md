...
1. **What is a Wide IP?** **Answer:** A service-level DNS name whose policy selects eligible destinations, rather than a proxy connection to the destination.
...
2. **Why distinguish server and virtual server?** **Answer:** A server groups an endpoint system; virtual servers identify reachable application addresses and ports on it.
...
3. **What does a monitor prove?** **Answer:** Only that a particular test succeeded from a particular observation point at that time.
...
4. **What is topology steering?** **Answer:** A rule that maps requester or resolver context to a preferred location or pool.
...
5. **How does G-A differ from ratio?** **Answer:** G-A prioritizes configured availability order and falls back; ratio distributes eligible selections by relative weights.
...
6. **Why does failover appear slow?** **Answer:** Recursive and client caches can retain the old answer until TTL and local caching rules permit refresh.
...
7. **Can BIG-IP DNS repair a dead application?** **Answer:** No. It can stop advertising an endpoint when evidence says it is unavailable; the client still connects elsewhere.
...
8. **Why test DNS over TCP?** **Answer:** Large or truncated responses and some DNSSEC/EDNS situations require TCP fallback.
...
9. **What makes DNS HA real?** **Answer:** Independent delegated nameserver paths, healthy listeners, synchronized state, resilient routing, and tested cache convergence.
...
10. **Why can “nearest” disappoint?** **Answer:** Resolver location may not represent the end user, and latency measurements may not match the client path.
