...
1. **What does a switch learn?** **Answer:** It learns a source MAC, ingress port, and VLAN association from observed frames; it does not learn an IP route from ordinary MAC learning.
...
2. **Why does a remote packet use the gateway MAC?** **Answer:** Ethernet is local-hop delivery. The IP destination remains remote, while the frame targets the next hop on the local link.
...
3. **Is ARP used by IPv6?** **Answer:** No. IPv6 uses ICMPv6 Neighbor Discovery, including Neighbor Solicitation and Advertisement. The IPv6 mechanism is neighbor discovery over ICMPv6, not an ARP table.
...
4. **What is an access port?** **Answer:** It is an endpoint-facing port assigned to one VLAN, commonly transmitting endpoint frames untagged.
...
5. **What is a trunk?** **Answer:** It is a link carrying multiple VLAN contexts, commonly represented with 802.1Q tags. The allowed set still must be checked.
...
6. **Why are native VLAN mismatches risky?** **Answer:** An untagged frame can be assigned to different VLANs at each end, causing leakage or loss and sometimes confusing control-plane behavior.
...
7. **Why can small packets work when large packets fail?** **Answer:** The path may have a lower effective MTU, and the mechanism that should communicate that fact may be blocked or mishandled.
...
8. **What does MAC flapping suggest?** **Answer:** A source MAC is being observed on changing ports, which can indicate a loop, mobility, teaming issue, spoofing, or a topology/configuration error. [Inference: Correlate the timestamps and VLAN before declaring an attack.]
